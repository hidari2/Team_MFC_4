...
{
	if (FAILED(CTextureMgr::Get_Instance()->InsertTexture(L"../Texture/Stage/Terrain/Tile/Tile%d.png", TEX_MULTI, L"Terrain", L"Tile", 124)))
	{
...
D3DXVECTOR3 CTerrain::Get_Tilepos(const D3DXVECTOR3 & vPos)
{
	int		iIndex = Get_TileIndex(vPos);

	D3DXVECTOR3 Tilepos=m_vecTile[iIndex]->vPos;
	return Tilepos;
}
