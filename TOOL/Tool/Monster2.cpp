...
void CMonster2::Initialize(D3DXVECTOR3 _pos)
{
	if (FAILED(CTextureMgr::Get_Instance()->InsertTexture(L"../Texture/Stage/Monster/Monster1/Dir_B/%d.png", TEX_MULTI, L"Monster1", L"Dir_B", 1)))
	{
...
void CMonster2::Render(void)
{
	...
	for (auto& iter : m_vecMonster)
	{
		...
		const TEXINFO*		pTexInfo = CTextureMgr::Get_Instance()->Get_Texture(L"Monster1", L"Dir_B", 0);
...
void CMonster2::Set_Ratio(D3DXMATRIX* pOut, const float& _fX, const float& _fY, const float& fZ)
{
	pOut->_11 *= _fX;
	pOut->_21 *= _fX;
	pOut->_31 *= _fX;
